 ...
 **/
define(['jquery', 'backbone', 'StackView', 'TweenLite'], function ($, Backbone, StackView, TweenLite) {
    ...
    StackView.ViewPort = BB.View.extend({
        ...
        addView: function (i_view) {
            i_view.$el.hide();
            // TweenLite.to(i_view.$el, 0.01, {autoAlpha: 0, display:'none'});
    ...
    StackView.Slider = StackView.ViewPort.extend({
        ...
        selectView: function (i_view) {
            ...
            $.each(self.m_views, function (id, view) {
                view.$el.hide();
                // TweenLite.to(view.$el, 0.01, {autoAlpha: 0, display:'none'});
            });
            i_view.$el.fadeIn(this.m_duration);
            // var duration = self.m_duration ? self.m_duration / 1000 : 1;
            // TweenLite.to(i_view.$el, duration, {autoAlpha: 1, display:'block'});
        },
    ...
    StackView.Fader = StackView.ViewPort.extend({
        ...
        /**
         Override base class addView so we can use TweenLite hide instead of jQuery hide
         @method addView
         @param {Object} i_view add a backbone view to control
         @return {String} stack view id added
         **/
        ...
        selectView: function (i_view) {
            ...
            var duration = self.m_duration ? self.m_duration / 1000 : 1;
            TweenLite.to(bb_view.$el, duration, {autoAlpha: 1, display:'block'});
        },
